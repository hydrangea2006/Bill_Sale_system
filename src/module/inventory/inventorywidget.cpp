...
void InventoryWidget::setupUI(int role)
{
    ...
    QStringList headers = {"ID", "Name", "Category", "Price", "Stock", "Unit"};
...
void InventoryWidget::onAddClicked()
{
    ...
    connect(submitBtn, &QPushButton::clicked, [&]() {
        if (nameEdit->text().isEmpty()) {
            QMessageBox::warning(&dialog, "Hint", "Please enter product name");
            return;
        }
        QVariantMap product;
...
void InventoryWidget::onEditClicked()
{
    ...
    if (currentRow < 0) {
        QMessageBox::warning(this, "Hint", "Please select a product to edit");
        return;
    }
...
void InventoryWidget::onDeleteClicked()
{
    int currentRow = m_tableWidget->currentRow();
    if (currentRow < 0) {
        QMessageBox::warning(this, "Hint", "Please select a product to delete");
        return;
    }
    ...
    QMessageBox::StandardButton reply = QMessageBox::question(
        this, "Confirm Delete",
        QString("Are you sure you want to delete product「%1」?").arg(productName),
...
{
    if (success) {
        QMessageBox::information(this, "Register Success", "Admin registration successful, please relogin");
